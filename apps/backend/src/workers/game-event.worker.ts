    ...
    async (jobs) => {
      for (const job of jobs) {
        ...
        try {
          ...
          } else if (result.reason === "Game already has pending action" || result.reason === "Game is paused") {
            // Game is paused or another event is pending — throw so pg-boss retries later
